...
class BoundaryRetryEventTest {
    ...
    fun errorBoundaryRetryRecoversAfterActionFailure() = runTest {
        val key = StackKey(hashCode())
        val stack = mutableListOf("Kotlin")

        val root = KineticaTest.render {
            val stackResource = resource(key, scope = CacheScope.Component) { _ -> stack.toList() }
            ...
            var submission by state(key = "submission") { Submission(0, "") }
