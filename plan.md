...
**Status: Gate met on branch `mem-opt-experiments` (8 commits, unmerged, pending review) — see KNT-0033b.** Re-profiling (2026-07-07, same-session Chrome 150 sweep) found the real driver was per-row allocation count, not the shadow tree: ~73 retained JS objects/row (two `Frame`s, six 1-entry hash maps, four atomicfu wrappers, ten closures) rather than tree-shape overhead. Cutting that to ~33 objects/row across 8 changes brought after-1k heap from 5.7MB to **2.96MB** (React 2.78MB, vanilla 0.75MB on the same pass) — inside the ≤5MB gate with margin. The whole-subtree-hoisting candidate below was never implemented; the gate closed through allocation-count reduction instead, so it's no longer needed unless a future regression reopens the gap.
- ~~Candidate if a gap remains: whole-subtree hoisting of fully-static host trees WITH children (the compiler stream stopped at leaf hosts + single-dynamic-text templates) — one shared immutable `Node` subtree across all rows/renders shrinks both allocation and retained shadow tree.~~ Superseded — not needed given the above.
...
**Status: Root-caused, fix implemented — branch `mem-opt-experiments` (8 commits, unmerged, pending review).** The KNT-0038 `ListenerRegistration` theory in `performance.md`/`03d3855` was **wrong**: profiling found zero `ListenerRegistration`s allocated per row in the benchmark (the row never calls `observe()`) and the theory can't explain startup anyway (zero rows exist at mount). The "startup regressed" claim itself was a **cross-session measurement artifact** — the doc compared a freshly-measured Kinetica against a stored, different-session React part; same-session interleaved measurement shows Kinetica startup *beating* React (13.4ms vs 18.5ms on a fresh Chrome 150 sweep, both fresh). The real heap cost was allocation count: ~73 retained JS objects/row (two `Frame`s per `each` row where one is a near-empty wrapper, six 1-entry hash maps each backed by an `Int32Array` bucket pair, four `atomicfu` wrapper objects, ten closures), largely structural to the frame-ordinals model, not the soundness fixes specifically.
- Fixed via 8 changes on `mem-opt-experiments`: eliding a dead `slotTouch` array on non-transient frames, packing `FrameSkipCache`'s dependency map into parallel arrays, collapsing the per-element browser event-binding expando (the single biggest win — per-element `LinkedHashMap`s were pure overhead in the common one-event case), dropping a now-redundant `MountedTemplate.eventBindings` array, **collapsing the two per-row `Frame`s into one** (compiler change: `each`/`lazyEach` row content no longer opens a second region frame — the keyed frame IS the region frame), compiler-fusing `hostEvent(onEvent = event{})` into a direct-block host event, replacing a per-cell `onWrite` closure with a shared hook, and eliminating the `atomicfu` wrapper objects on JS.
- The last item is a reusable finding: **atomicfu's own compiler plugin cannot elide on JS in this toolchain** — its `isJs()` platform guard rejects the toolchain's combined js+wasmJs `TargetPlatform`, so the plugin silently no-ops on JS while working on JVM (proven: a local `atomic()` errors the JVM compile, compiles clean on JS). `kinetica-compiler`'s own `IrGenerationExtension` has no such guard, so a purpose-built `KineticaAtomicUnwrapLowering` gated on `!isJvm() && !isNative()` succeeds where atomicfu's plugin can't — atomics unwrap to plain fields on JS/wasm, stay real on JVM/Native (concurrency tests still pass).
- Result: after-1k heap 5.7MB → **2.96MB** (React 2.78MB) — was 2.05× React, now ~1.07×; 13-op geomean 1.23× → **1.19×**, now ahead of React (1.24×) and Vue (1.22×). All soundness suites green throughout (compiler, runtime JVM incl. concurrency tests, browser + kinetica-test JS), leak-checked. `performance.md`'s Kinetica column/narrative were deliberately left untouched pending this branch's review — see the 2026-07-07 "Chrome 150" table-refresh commit for the non-Kinetica baseline update.
...
1. The backlog is unscheduled. Per-ticket starting points: KNT-0024 → re-profile, KNT-0028/KNT-0035 → spec decision, KNT-0036 → design decision. (KNT-0033/0034/0038 landed 2026-07-07 via the codex-TDD pipeline; KNT-0031/0033b root-caused and fixed 2026-07-07 on `mem-opt-experiments`, pending merge review.)
