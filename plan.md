...
**Status:** Code landed 2026-07-21 on `main` (`ba34c2d` linuxX64 platforms + src@native consolidation, `06d0101` renderer/sample/CI lane) — **pending first green `linux` CI run**: kinetica-gtk cannot compile on macOS (no GTK headers), so the cinterop/signal surface is CI-validated only. Spike verdict: gtk-kn REJECTED (0.0.3-SNAPSHOT only, Kotlin 2.1.0, requires its Gradle plugin — incompatible with the Amper toolchain); flags come from `generate-def.sh` (shell-expanded pkg-config, gitignored def). Local full builds on macOS now include kinetica-gtk in project.yaml — `./kotlin build` without `-m` will fail on it here; use module-scoped builds (CI jobs already are).
