...
**Status:** Done (2026-07-07, codex TDD, with KNT-0017) — `sourceText`/`hasExplicitNullArgument` deleted (IR facts suffice: absent slot rejected, explicit IrConst null accepted); props-less AND zero-arg `propsOf()` hosts template as empty-props; per-file `ConstPropsFieldIndex` owned by the extension, passed to both transformers, `IrGetField` recognition index-only. Order-independence proven by a reversed-order harness compile through the real plugin (test-only `io.heapy.kinetica.compiler.irTransformOrder` system property). Compiler 68/68, runtime 181/181, annotated JVM+JS green.
...
**Status:** Done (2026-07-07, with KNT-0009) — `KineticaIrShared.kt`: `ConstPropsFieldIndex` (contract KDoc), `constPropsOf` (fqName check; empty accepted — hoist rejects empty at its call site), `addStaticFileField` (attribute-identical to both deleted copies), shared `isNullConst` (frames' duplicate removed too).
